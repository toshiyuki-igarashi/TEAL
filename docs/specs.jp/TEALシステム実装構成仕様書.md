...
ポリシーエンジンの判定対象となる subject オブジェクトに対し、カーネルおよびPAMから得られる以下のコンテキスト属性をリアルタイムにバインド（Enrich）する。属性情報の信頼境界（Trust Boundary）は下表の通り定義される。
...
3. **デーモン側での交差照合:** teald のポリシーエンジンは、カーネルから直達した TEAL\_ATTR\_SESSION\_TTY と、PAMモジュールから受信した該当 UID のアクティブなセッションキャッシュを突き合わせる。両者が一致しない場合、またはTTYが None（非対話型のWebシェル等）である場合は、たとえ提示された秘密鍵が正当でありroot権限を有していても、即座に要求を DENY（強制遮断）する。

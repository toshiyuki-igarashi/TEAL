...
## 5. インターフェース実装仕様 (TLV/バイナリ通信プロトコル)
...
### 5.1 Netlink コマンド定義 (Message Types)
...
### 5.2 Netlink 属性定義 (Attributes / TLV)

メッセージに付与される個々のデータフィールド（TLVのType）を定義する。
...
| `TEAL_ATTR_TARGET_DEV` | `u32` | 操作対象のデバイス番号 |
| `TEAL_ATTR_TARGET_INO` | `u64` | 操作対象のInode番号 |
| `TEAL_ATTR_TARGET` | `String` | 操作対象の絶対パス |
...
| **`TEAL_ATTR_APPLET`** | `String` | **マルチコールバイナリ用識別名 (`current->comm`)** |
|      |      | ※区切り文字衝突問題はTLV化により解消 |
| `TEAL_ATTR_LSM_LABEL`| `String` | SELinux等のラベル |
|      |      | ※バイナリ化に伴い16進数エンコード(`LSM_LABEL_HEX`)は**不要とし、生文字列を送る** |
| `TEAL_ATTR_ARGS_HEAD`| `String` | 引数の先頭部分 （最大128 bytes, Optional） |
|      |      | ※ `TEAL_ATTR_ARGS_HEAD` は完全な引数列ではなく、SIEM連携および一次トリアージ用の軽量な要約フィールドである。引数列を正規化した文字列表現の先頭 128 bytes のみを格納する。完全な証跡を目的とするものではない。 |
...
#### 5.5 データ取得責務分界 (Data Responsibility Matrix)
...
## 6. 監査ログと証跡管理 (Audit & Evidence)

### 6.1 リクエストID体系と役割 (Identity Strategy)
...
### 6.2 パス解決とログエンリッチメント (Path Resolution Strategy)
...
#### 6.3 データ構造 (A) Fast Path Log Schema (Ticket Consumed)
...
### 6.4 リクエストID生成と管理 (Request ID Strategy)
...
### 6.5 データ構造 (B) Slow Path Log Schema (Interactive Decision)
...
### 6.6 監査チェーン (Audit Chain) と正規化モデル
...
### 6.7 コンテキスト解決とハッシュ検証 (Context Resolution)
...
### 6.8 コマンドライン引数の記録戦略 (Selective Argument Audit)
...
### 6.9 管理操作の監査 (Auditing Administrative Operations)
...
### 7.1 デュアルレーン・アーキテクチャ (Kernel-User Communication)
...
### 7.3 ハイブリッド運用（Selective Audit in Enforce Mode）
...
## 9. ポリシー設定とパス照合ロジック (Policy Matching)
...
### 9.4 キャッシュと有効期限設定 (Caching & TTL Strategy)
...
### 9.4.1 監査レベル（Audit Level）のポリシー定義
...
### 10.2. サブジェクト定義の拡張（Subject Enrichment）
...
### 10.3. 環境情報の解決メカニズム（Ancestry Walk）
...
### 10.4. 時間軸による制御と猶予期間（Time-Window & Grace Period）
...
### 11.4 監視とエスカレーション (Logging & Alert)
...
### 13.2 閾値暗号と分散承認 (Threshold Cryptography)
...
### 13.4 ハードウェア Root of Trust (TPM Integration)
...
#### 13.5.2 カーネル内ログ重複排除・レートリミット（AUDITモードのフェイルセーフ）
...
### 13.8 ログ管理の高度化と外部連携（将来拡張）

#### 13.8.1 ログファイル命名規則の標準化
